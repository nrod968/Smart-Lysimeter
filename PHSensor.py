...
def read_line():
	...
	"""
	line_sep_len = len(b'\r') # gets length of the line terminator
	read_buffer = [] # read buffer
	while True: # read byte by byte until line terminator is read
		next_char = ser.read(1) # reads one character
		if next_char == b'':
			break
		read_buffer.append(next_char)
		if (len(read_buffer) >= line_sep_len and
				read_buffer[-line_sep_len:] == [b'\r']):
			break
	return b''.join(read_buffer)
